...
export async function syncBountyEvaluation(bountyId: string) {
  const { rows: bountyRows } = await pool.query("select * from bounties where id = $1", [bountyId]);
  ...
  await pool.query(`update bounties set status = $2, reward_claimed = $3, updated_at = now() where id = $1`, [
    bountyId,
    dbStatus,
  ...
  for (const chainTestimonyId of testimonyIds) {
    ...
    await pool.query("update testimonies set status = $2, consistency_bps = $3 where chain_testimony_id = $1", [
      chainTestimonyId,
      newStatus,
  ...
  if (dbStatus === "resolved" && evaluationId) {
    ...
    const { rows: existingRecord } = await pool.query("select id from truth_records where bounty_id = $1", [
      bountyId,
    ]);
